...
void NodeNetwork::sendDataToPeer(int id, LLint* data, int start, int amount, uint size, uint flag){
	try{
        	uint read_amount = 0;
...
void NodeNetwork::sendDataToPeer(int id, uint size, LLint* data, uint flag)
{
    uint count = 0, rounds = 0;
    getRounds(size, &count, &rounds, flag);
    for(int k = 0; k <= rounds; k++)
...
void NodeNetwork::SendAndGetDataFromPeer(int sendtoID, int RecvFromID, LLint* SendData, LLint* RecvData, uint size, uint flag){
    //compute the maximum size of data that can be communicated
    uint count = 0, rounds = 0;
    getRounds(size, &count, &rounds, flag);
    // printf("rounds = %u\n", rounds);
...
void NodeNetwork::getDataFromPeer(int id, uint size, LLint* buffer, uint flag)
{
    uint count = 0, rounds = 0;
    getRounds(size, &count, &rounds, flag);
    memset(buffer,0,sizeof(Lint)*size);
...
void NodeNetwork::getDataFromPeer(int id, LLint* data, int start, int amount, uint size, uint flag){
	try{
            	int write_amount = 0;
            	if(start+amount > size)
                	write_amount = size-start;
...
void NodeNetwork::multicastToPeers(LLint** data, LLint** buffers, uint size, uint flag){
    int id = getID();
    int peers = config->getPeerCount();
