...
void Rss_normal(Lint *b, Lint **v, Lint *p, Lint **vp, int w, int beta, int m, uint ring_size, int pid, int *map, NodeNetwork *nodeNet)
{
  ...
  new_Rss_MSB(tmp_b, tmp, 1, ell, map, nodeNet); // new
  ...
  // mult
  Rss_Mult(tmp_c, tmp, tmp_b, 1, ell, map, nodeNet);


  // BitDec
  Rss_bitDec3(bits, tmp_c, 1, ell, w*beta, map, nodeNet); //TODO check ring size
  ...
  dp[0][ell - 1] =  bits_tmp[0][ell - m - 1];
  dp[1][ell - 1] =  bits_tmp[1][ell - m - 1];

  for(i = m; i <= ell - 2; ++i) {
    dp[0][i] = bits_tmp[0][i - m] - bits_tmp[0][i - m + 1]; //TODO: Double check here
    dp[1][i] = bits_tmp[1][i - m] - bits_tmp[1][i - m + 1];
  }
  ...
  Rss_ORead2(u, bitsp, dp, ell, m, ring_size, map, nodeNet);
  ...
  for (i = w * (beta-2); i < m; ++i) {
    v[0][beta - 1] += u[0][i] << (i - w*(beta-2));
    v[1][beta - 1] += u[1][i] << (i - w*(beta-2));
  }
