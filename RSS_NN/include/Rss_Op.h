...
template <typename T>
void Rss_MatMult(T*** c, T*** a, T*** b, uint m, uint n, uint s, uint ring_size,  int *map, NodeNetwork *nodeNet) {
    ...
    for(i = 0; i < m; i++){
        v_a[i] = new T [s];
        memset(v_a[i],0,sizeof(T)*s);
    }

    T *send_buf = new T [m*s];
    memset(send_buf,0,sizeof(T)*m*s);
    T *recv_buf = new T [m*s];
    memset(recv_buf,0,sizeof(T)*m*s);
...
template <typename T>
void Rss_MatMultArray(T** c, T** a, T** b, uint m, uint n, uint s, uint ring_size, int *map, NodeNetwork *nodeNet){
    ...
    //send v_a
    for(i = 0; i < m; i++){
        // memcpy(send_buf+i*s, v+i, sizeof(T)*s);
        memcpy(send_buf+i*s, v+i*s, sizeof(T)*s);
    }
    nodeNet->SendAndGetDataFromPeer(map[0], map[1], send_buf, recv_buf, m*s, ring_size);

    for(i = 0; i < m; i++){
       memcpy(c[1] + i*s, recv_buf+i*s, sizeof(T)*s); // PROBLEM HERE
    }
...
template <typename T>
void Rss_MatMultArray_batch(T** c, T** a, T** b, uint m, uint n, uint s, uint ring_size, uint batch_size, uint weight_flag_a, uint weight_flag_b, int *map, NodeNetwork *nodeNet){
    // modified implementation that uses a 1d array representation of a matrix
    // a [m*n] = i*n+j (accessing the (i,j)th element)
    // [m][n] size matrix a;   [n][s] size matrix b
    // ---> [m][s] size matrix c;
    ...
    T *v = new T [batch_size*m*s];
    memset(v,0,sizeof(T)*(batch_size*m*s));


    T *send_buf = new T [batch_size*m*s];
    memset(send_buf,0,sizeof(T)*batch_size*m*s);
    T *recv_buf = new T [batch_size*m*s];
    memset(recv_buf,0,sizeof(T)*batch_size*m*s);
    ...
    for (l = 0; l < batch_size; l++) {
        ...
        for(i = 0; i < m; i++){
            // memcpy(send_buf+i*s, v+i, sizeof(T)*s);
            memcpy(send_buf + i*s + l*m*s, v + i*s + l*m*s, sizeof(T)*s);
        }
    }
    ...
    for (l = 0; l < batch_size; l++) {
        for(i = 0; i < m; i++){
           memcpy(c[1] + i*s + l*m*s, recv_buf+i*s + l*m*s, sizeof(T)*s); // PROBLEM HERE
        }
...
/*************Randbit****************/
template <typename T>
void Rss_RandBit(T** b, uint size, uint ring_size, int *map, NodeNetwork* nodeNet){
    ...
    rss_sqrt_inv<T>(c, e, size, ring_size+2);
...
template <typename T>
void Rss_RandBit3(T** b, uint size, uint ring_size, int *map, NodeNetwork* nodeNet){
...
template <typename T>
void Rss_edaBit(T **r, T **b_2, uint size, uint ring_size, int *map, NodeNetwork *nodeNet) {
...
//For generaing bit_length edabits, where bit_length is smaller than ring_size
template <typename T>
void Rss_edaBit(T **r, T **b_2, uint size, uint ring_size, uint bit_length, int *map, NodeNetwork *nodeNet) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        b_carry[i] = new T[size];
        memset(b_carry[i], 0, sizeof(T) * size);
        a_carry[i] = new T[size];
        memset(a_carry[i], 0, sizeof(T) * size);
        // ensuring destinations are sanitized
        memset(r[i], 0, sizeof(T) * size);
        memset(b_2[i], 0, sizeof(T) * size);
    }
...
template <typename T>
void Rss_GenerateRandomShares(T **res, T **res_bitwise, uint ring_size, uint bit_length, uint size, int *map, NodeNetwork *nodeNet) {
   int pid = nodeNet->getID();
    uint i, j;
    uint bytes = (ring_size + 7) >> 3;
    // printf("bytes : %u \n", bytes);
...
// void Rss_edaBit_trunc(T **r, T **r_prime, T **b_2, uint size, uint ring_size, uint m, int *map, NodeNetwork *nodeNet) {
template <typename T>
void Rss_edaBit_trunc(T **r, T **r_prime, T **r_km1, uint size, uint ring_size, uint m, int *map, NodeNetwork *nodeNet) {
    ...
    memcpy_fast(r_km1[0], carry[0] + 2 * (size), size * sizeof(T));
    memcpy_fast(r_km1[1], carry[1] + 2 * (size), size * sizeof(T));
...
template <typename T>
void Rss_edaBit_truncPre(T **x, T *c, T **bitLTres, T **r, T **r_prime, T **r_km1, uint size, uint ring_size, uint m, int *map, NodeNetwork *nodeNet) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        memset(r_bitwise[i], 0, sizeof(T) * new_size);
        // carry will hold both kth and m-1th bits, in succession
        carry[i] = new T[b2a_size + size];
        memset(carry[i], 0, sizeof(T) * (b2a_size + size));
	...
	b_prime[i] = new T[size];
        memset(b_prime[i], 0, sizeof(T) * size);
    }
    ...
    memcpy_fast(&carry[0][b2a_size], bitLTres[0], sizeof(T) * size);
    memcpy_fast(&carry[1][b2a_size], bitLTres[1], sizeof(T) * size);
    ...
    memcpy_fast(bitLTres[0], &carry[0][b2a_size], sizeof(T) * size);
    memcpy_fast(bitLTres[1], &carry[1][b2a_size], sizeof(T) * size);
...
template <typename T>
void Rss_GenerateRandomShares_trunc(T **res, T **res_prime, T **res_bitwise, uint ring_size, uint m, uint size, int *map, NodeNetwork *nodeNet) {
...
// k' --> ring_size_prime
template <typename T>
void Rss_Convert(T **res, T **a, uint size, uint ring_size, uint ring_size_prime, int *map, NodeNetwork *nodeNet) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        a_2[i] = new T[size];
        sum[i] = new T[size];
        a_k_prime[i] = new T[(n_rand_bits >> 1)];
    }

    for (j = 0; j < size; j++) {
        for (k = 0; k < ring_size; k++) {
            index = j * ring_size + k;
            r[0][j] = r[0][j] + (r_shares[0][index] << T(k));
            r[1][j] = r[1][j] + (r_shares[1][index] << T(k));
        }
    }
    for (j = 0; j < size; j++) {
        for (k = 0; k < ring_size; k++) {
    ...
    for (j = 0; j < size; j++) {
        for (k = 0; k < ring_size; k++) {
            ...
            r_2[0][j] = T(SET_BIT(r_2[0][j], T(k), GET_BIT(r_shares[0][size * ring_size + index], T(0))));
            r_2[1][j] = T(SET_BIT(r_2[1][j], T(k), GET_BIT(r_shares[1][size * ring_size + index], T(0))));
        }
    }
    ...
    for (i = 0; i < size; i++) {
        for (j = 0; j < ring_size; j++) {
            ...
            a_k_prime[0][index] = a1 * GET_BIT(c[i], T(j)) + r_shares[0][size * ring_size + index] - 2 * GET_BIT(c[i], T(j)) * r_shares[0][size * ring_size + index];
            a_k_prime[1][index] = a2 * GET_BIT(c[i], T(j)) + r_shares[1][size * ring_size + index] - 2 * GET_BIT(c[i], T(j)) * r_shares[1][size * ring_size + index];
        }
    }
    ...
    for (j = 0; j < size; j++) {
        ...
        for (k = 0; k < ring_size; k++) {
            ...
            res[0][j] += (a_k_prime[0][index] << T(k));
            res[1][j] += (a_k_prime[1][index] << T(k));
...
/********************BitAddTrunc*******************/
template <typename T>
void Rss_nBitAdd_trunc(T **res, T **carry, T **r_bitwise, uint ring_size, uint m, uint size, int *map, NodeNetwork *nodeNet) {
    ...
    for (j = 0; j < rounds; j++) {
        ...
        if (j == 0) {
            ...
            memcpy(a[0], r_bitwise[0], size * sizeof(T));
            memcpy(a[1], r_bitwise[1], size * sizeof(T));

            memcpy(b[0], r_bitwise[0] + size, size * sizeof(T));
            memcpy(b[1], r_bitwise[1] + size, size * sizeof(T));
...
template <typename T>
void Rss_BitAdd_trunc(T **res, T **carry, T **a, T **b, uint ring_size, uint m, uint size, int *map, NodeNetwork *nodeNet) {
    ...
    for (i = 0; i < size; i++) {
        ...
        // m-1th carry bit
        carry[0][i] = GET_BIT(d[2][i], T(m - 1));
        carry[1][i] = GET_BIT(d[3][i], T(m - 1));

        // kth carry bit
        carry[0][size + i] = GET_BIT(d[2][i], T(ring_size-1));
        carry[1][size + i] = GET_BIT(d[3][i], T(ring_size-1));

        // getting MSB of res = a + b
        carry[0][2 * size + i] = GET_BIT(res[0][i], T(ring_size - 1));
        carry[1][2 * size + i] = GET_BIT(res[1][i], T(ring_size - 1));
    }
...
template <typename T>
void Rss_bitDec3in2(T ***res, T **a, uint size, uint ring_size, int *map, NodeNetwork *nodeNet) {
    ...
    for(i = 0; i < 2; ++i) {
      for(j = 0; j < size; ++j) {
	for(k = 0; k < ring_size; ++k) {
	  //	  b2ainput[i][j*ring_size + k] = a_2[i][j] & (1 << k);
	  res[i][j][k] = GET_BIT(a_2[i][j], (T)k);
	}
      }
    }
...
template <typename T>
void Rss_bitDec3(T ***res, T **a, uint size, uint ring_size, int *map, NodeNetwork *nodeNet) {
    ...
    for (i = 0; i < 2; i++) {
      b2ares[i] = new T[size * ring_size]();
      b2ainput[i] = new T[size * ring_size]();
    }
	...
	for(k = 0; k < ring_size; ++k) {
	  //	  b2ainput[i][j*ring_size + k] = a_2[i][j] & (1 << k);
	  b2ainput[i][j*ring_size + k] = GET_BIT(a_2[i][j], (T)k);
	}
      }
    }

    //    Rss_reveal(b2ainput, ring_size, size * ring_size, map, nodeNet);
    Rss_b2a3(b2ares, b2ainput, ring_size, size * ring_size, map, nodeNet);
    //Rss_reveal(b2ares, ring_size, size * ring_size, map, nodeNet);

    for(i = 0; i < 2; ++i) {
      for(j = 0; j < size; ++j) {
	for(k = 0; k < ring_size; ++k) {
	  res[i][j][k] = b2ares[i][j*ring_size + k];
	}
      }
    }
...
void Rss_bitDec3(T ***res, T **a, uint size, uint ring_size, uint bitLen, int *map, NodeNetwork *nodeNet) {
    ...
    // only need ring_size bit-length values for both parts of computation
    // first protection and b2a
    Rss_edaBit(edaBit_r, edaBit_b_2, size, ring_size, bitLen, map, nodeNet);
    ...
    // Rss_Open(c, sum, size, map, ring_size_prime, nodeNet);
    Rss_Open(c, sum, size, map, ring_size, nodeNet); //TODO: bitLen or ring_size
...
/*************B2A**************/
template <typename T>
void Rss_b2a(T **res, T **a, uint ring_size, uint size, int *map, NodeNetwork *nodeNet) {
    int pid = nodeNet->getID();
    uint i;
    T **b = new T *[2];
    T **b_2 = new T *[2];
    ...
    for (i = 0; i < size; i++) {
        b_2[0][i] = GET_BIT(b[0][i], T(0));
        b_2[1][i] = GET_BIT(b[1][i], T(0));
    }
...
void Rss_b2u(T ***res, T **a, uint ring_size, uint size, uint m, int *map, NodeNetwork *nodeNet) {
  //size = batch_size
  int pid = nodeNet->getID();
  uint i, j;
  int q = ceil(log2(m));
  ...
  for (i = 0; i < 2; i++) {
    r[i] = new T[size*q];
    bits[i] = new T*[q];
    allorRes[i] = new T*[allorResSize];
    for(j = 0; j < q; ++j) {
      bits[i][j] = new T[size];
    }
    for(j = 0; j < allorResSize; ++j) {
      allorRes[i][j] = new T[size];
    }
  }
  ...
  for(i = 0; i < 2; ++i) {
    for(j = 0; j < q; ++j) {
      memcpy(&bits[i][j][0], &r[i][j*size], sizeof(T)*size);
    }
  }
  //allor
  ...
  T **open_tmp1 = new T *[2];
  T *c = new T[size];
  open_tmp1[0] = new T [size];
  open_tmp1[1] = new T [size];
  memset(open_tmp1[0], 0, sizeof(T)*size);
  memset(open_tmp1[1], 0, sizeof(T)*size);
...
void Rss_eqz3in2(Lint **res, Lint **a, uint size, uint ring_size, int *map, NodeNetwork *nodeNet);
