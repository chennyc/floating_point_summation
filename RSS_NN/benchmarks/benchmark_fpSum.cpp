...
delete[] x[i][j];
  ...
  */
  ...
  printf("total comm. = %lf MB\n", (double)(nodeNet->getCommunicationInBytes()) / rep / 1024 / 1024);
...
/*
{
  ...
  Rss_reveal(a, ring_size, batch_size,  map, nodeNet);
  for(i = 0; i < rep; ++i){
    Rss_bitDec3(bitDecRes, a, batch_size, ring_size, 3, map, nodeNet);
  }
  gettimeofday(&end, NULL);
  timer = 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;
